...
export async function transcribeAudio(audioAsBase64: string, mimeType: string){
    const response = await gemini.models.generateContent({
        ...
        contents: [
            {
                text: 'Transcreva o áudio para português do Brasil. Seja preciso e natural na transcrição. Mantinha a pontuação adequada e divida o texto em parágrafos quando for apropriado.'
            },
            {
                inlineData:{
                    mimeType,
...
export async function generateAnswer(question:string, transcriptions: string[]){
    ...
    INSTRIÇÕES:
     ...
     - Se for citar o contexto, chame-o utilize o termo "conteúdo da aula";
    `.trim()
